...
namespace EmailScheduler.DAL
{
    public static class DatabaseManager
    {
        ...
        public static void UpdateTasks(ITask task)
        {
            using (var context = new SchedulerTasksDataContext(CONNECTIONSTRING))
            {
                var results = context.spUpdateTasks(task.Id, task.UserInfo.username, task.TaskType, task.Title, task.Status, task.ScheduledTime, task.Frequency.ToString());
            }
        }

        public static void InsertAccount(UserContext user)
        {
            using (var context = new SchedulerTasksDataContext(CONNECTIONSTRING))
            {
